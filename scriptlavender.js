...
renderer.setClearColor(0x000000, 0);  // 0x000000 is black, but '0' is the transparency level
...
let model;  // Declare the model variable outside for global access

const loader = new THREE.GLTFLoader();
loader.load(
    'free_-_skybox_lavender_fields.glb',  // Path to your GLB file
    function (gltf) {
        model = gltf.scene;
...
// Animation loop to render the scene and auto-rotate the model
function animate() {
    requestAnimationFrame(animate);
    
    // If the model is loaded, apply auto-rotation
    if (model) {
        model.rotation.y += 0.01;  // Adjust rotation speed (y-axis rotation)
    }
